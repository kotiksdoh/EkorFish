  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (productId) {
        ...
        const { activeProductId, product, isLoadingProduct } =
          store.getState().catalog;
        const hasCurrentData =
          String(activeProductId) === id &&
          product &&
          String(product.id) === id;
        const isLoadingThisProduct =
          isLoadingProduct && String(activeProductId) === id;

        if (!hasCurrentData && !isLoadingThisProduct) {
