...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  const [currentScenario, setCurrentScenarion] = useState<AuthScenario>(
    AuthScenario.DEFAULT,
  );
  const [currentScreen, setCurrentScreen] = useState<ScreensScenario>(
    ScreensScenario.ACC_TYPE,
  );
      ...
      (res: any) => {
        ...
        if (sendCode.fulfilled.match(res)) {
          ...
          if (res.payload.data.data.needUserType) {
            ...
            !res.payload.data.data.needUserType &&
            res.payload.data.data.needInformationForType === "Individual"
          ) {
            setCurrentScenarion(AuthScenario.REG_NEED);
            setCurrentScreen(ScreensScenario.USER_REG);
          } else if (
            !res.payload.data.data.needUserType &&
            res.payload.data.data.needInformationForType === "Legal"
          ) {
            setCurrentScenarion(AuthScenario.NEED_ACC_TYPE);
            setCurrentScreen(ScreensScenario.ACC_TYPE);
          } else if (
            !res.payload.data.data.needUserType &&
            (res.payload.data.data.needInformationForType === null ||
              res.payload.data.data.needInformationForType === undefined)
          ) {
  ...
  const codeInputBackgroundColor =
    currentTheme === "dark" ? "#ECEFFA0D" : "#F2F4F7";
  ...
  return (
    ...
    >
      <SafeAreaProvider>
        {currentScenario === "login" ? (
              ...
              <View style={styles.afterLogoContent}>
                {!showCodeInput ? (
                  ...
                  <>
                    {/* Заголовок */}
                    <ThemedText
                      style={styles.modalTitleAfterPhone}
                      lightColor={"#1B1B1C"}
                    >
                      Код подтверждения
                    </ThemedText>
                    ...
                    {/* Описание */}
                    <ThemedText
                      style={styles.modalDescriptionAfterPhone}
                      lightColor={"#80818B"}
                      darkColor="#FBFCFF80"
                    >
        ...
        ) : currentScenario === AuthScenario.NEED_ACC_TYPE &&
          currentScreen === ScreensScenario.ACC_TYPE ? (
          <ThemedView
            lightColor={"#EBEDF0"}
            darkColor="#040508"
          ...
          >
            ...
            />
            <ThemedView
              style={stylesAccType.modalContentInnerReg}
              lightColor={"#FFFFFF"}
            >
              ...
              <View style={stylesAccType.accountTypeContainer}>
                ...
                >
                  ...
                  <ThemedText
                    ...
                    style={[stylesAccType.accountTypeText]}
                  >
                    Частное лицо
                  </ThemedText>
                </TouchableOpacity>
        ...
        ) : currentScenario === AuthScenario.REG_NEED ||
          currentScreen === ScreensScenario.USER_REG ? (
          <ThemedView
            lightColor={"#EBEDF0"}
            darkColor="#040508"
            ...
            /> */}
              ...
              <View style={stylesRegUser.buttonUserReg}>
                <PrimaryButton
                  ...
                  fullWidth
                  disabled={
                    !birthDate || !secondName || !name || !surname || loading
                  }
                  style={stylesAccType.continueButton}
                />
              </View>
          ...
          >
            <ModalHeader
              title={
                predUserData?.needUserType === false &&
                predUserData?.needInformationForType === "Legal"
                  ? "Привязка к компании"
                  : "Регистрация"
              }
              // 'Привязка к компании'
            ...
            <ThemedView
              lightColor={"#FFFFFF"}
              style={[stylesRegUser.modalContentInnerRegUser, { flex: 1 }]}
            >
              <View>
                {/* <ThemedText style={stylesAccType.accountTypeTitle}>
              Данные вашей компании
                ...
                predUserData?.needInformationForType === "Legal" ? (
                  <ThemedText style={stylesSearchComp.textUp}>
                    Ваш номер телефона найден, но не привязан к компании.
          ...
          >
            ...
            />
            {
              ...
              company === null ? (
                <ThemedView
                  style={stylesError.modalContentErr}
                  lightColor={"#FFFFFF"}
                >
                  <View style={stylesError.errorContainer}>
                    <Image
                      ...
                      resizeMode="contain"
                    />
                    <ThemedText style={stylesError.mainErr}>Ошибка</ThemedText>
                    <ThemedText
                      lightColor="#80818B"
                      style={stylesError.secondErr}
                    >
                      Компания с таким ИНН не найдена или привязана к другому
                      номеру
                    </ThemedText>
                    {/* <View> */}
                    <PrimaryButton
